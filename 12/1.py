...
def recur_find_peri_area(i, j, input_arr, peri, area, ch, travelled_arr):
    #print(f"{i},{j}")
    if i < 0 or i == len(input_arr) or j < 0 or j == len(input_arr[0]):
        ...
        if input_arr[i][j] == ch:
            ...
            if [i, j] not in travelled_arr:
                ...
                peri1, area1 = recur_find_peri_area(i+1, j, input_arr, peri, area, ch, travelled_arr)
                peri2, area2 = recur_find_peri_area(i-1, j, input_arr, peri, area, ch, travelled_arr)
                peri3, area3 = recur_find_peri_area(i, j+1, input_arr, peri, area, ch, travelled_arr)
                peri4, area4 = recur_find_peri_area(i, j-1, input_arr, peri, area, ch, travelled_arr)
...
for k, line in enumerate(input_arr):
    for l, ch in enumerate(line):
        ...
        if check_if_ch_exists or not check_if_travelled:
            print(f"{ch} in {k},{l}")
            travelled_arr, area = recur_find_peri_area(k, l, input_arr, 0, 1, ch, [])
            #print(travelled_arr)
